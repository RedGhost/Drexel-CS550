...
(define (uniquely-asserted ops f-stream) (stream-flatmap (lambda (fr)
...
(define (idb rules-and-assertions)
  (put 'unique 'qeval uniquely-asserted) 
  (put 'and 'qeval conjoin) )
...
(rule (reverse (?head . ?tail) ?rev )
	(and (reverse2 ?tail ?revtail )
	     (append-to-form ?revtail (?head) ?rev) ) ) 

(rule (meeting-time ?person ?time)
	(or (meeting whole-company ?time)      
	    (and (meeting ?division ?time) (job ?person (?division . ?title)) )
	)
)


(rule (meeting-time ?person-a ?time)
      (or (meeting whole-company ?time)
